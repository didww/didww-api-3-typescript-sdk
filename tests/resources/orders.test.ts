...
describe('Orders', () => {
  ...
  it('creates an order', async () => {
    ...
    const result = await client.orders().create({
      ...
      items: [
        { type: 'did_order_items', sku_id: 'acc46374-0b34-4912-9f67-8340339db1e5', qty: 2 },
  ...
  it('creates an order with billing_cycles_count', async () => {
    ...
    const result = await client.orders().create({
      allow_back_ordering: true,
      items: [
        { type: 'did_order_items', sku_id: 'f36d2812-2195-4385-85e8-e59c3484a8bc', qty: 1, billing_cycles_count: 5 },
      ],
    });
  ...
  it('creates an order with callback', async () => {
    ...
    expect(result.data.id).toBe('5da18706-be9f-49b0-aeec-0480aacd49ad');
    expect(result.data.status).toBe('Pending');
    expect(result.data.callback_url).toBe('https://example.com/callback');
    expect(result.data.callback_method).toBe('POST');
    expect(result.data.items.length).toBe(1);
  });
});
